 ...
 * 基于 qrcodejs 改编，适配微信小程序 Canvas 2D API
 */
...
(function () {
  ...
  QRCode = function (options) {
    ...
    var context = this.options.usingIn;
      ...
      .exec((res) => {
        ...
        for (var row = 0; row < qrcode.getModuleCount(); row++) {
          for (var col = 0; col < qrcode.getModuleCount(); col++) {
            var style = qrcode.isDark(row, col) ? this.options.foreground : this.options.background;
            ctx.fillStyle = style;
            var x = col * tileW;
            var y = row * tileH;
